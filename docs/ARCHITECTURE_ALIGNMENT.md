...
## 31. 清醒的空闲与图文说说（2026-09-20 修订）

用户确认：清醒时倾向于主动留下下一次联系的时间，发说说时可自主选择同步配图。

- 空闲表示暂时不聊天、去过自己的生活，不再意味着停止心跳。未睡且本拍安静仍保留下一次唤醒；漏填时间时兜底，显式时间尊重模型规划。到期重新判断是否联系，可以继续安静。睡下及用户关闭心跳仍停止安排。
- 图文说说的准备阶段决定正文和是否分享画面，相机决定镜头与生成，空间负责上传/发布。跨器官流程由中枢编排，不给插件增加互调入口，不改潜意识或身份复盘的职责。
- 选了配图则准备完成后图文一次发布；图片准备失败不先发残缺文字。发布不自动重试，保持对实际副作用的既有处理边界。普通对话的发布闸门及空闲抽签日限保留。

状态：实现与离线回归完成；真实空间图文发布、模型表达自然度及主动联系频率待实际使用验证。

## 32. 错误保护与 QQ 系统通知（2026-09-20）

用户明确要求：复盘或对话失败时，QQ 在线应通知用户，同时不能在后台无限重试消耗额度。宿主据此增加持久化的任务/供应商暂停状态，失败耗尽有限请求后必须由用户恢复；它是运行安全边界，不由角色心智解除，也不改变人格、认知形成或记忆内容。

错误通知直接走平台动作，标明系统身份，不调用模型、不伪装角色发言、不写入日常对话。发送尝试先持久化去重，失败回执不自动重发。日构建失败不得自行从头无限补跑；并行批次首错终止余下工作。成功任务仍按既有计划运行，人工恢复后允许按原日序补跑；已有部分记忆产物不在本次修复中回滚或重写。


### 日构建入口的历史优先（2026-09-20 补充）

用户进一步要求日构建先检测历史错误、先完成旧日，再进行目标日。手动与自动入口必须共用历史优先顺序和整批互斥，不能由页面传参越过失败日。预检不调用模型，不清错误，不把失败标记改成成功；旧日只有实际构建成功且留下完成标记，后续日期才可执行。旧错误按尝试留存，以区分最初失败与后续额度耗尽。遗留失败需用户处理阻塞并显式开始，不因升级或重启自动重跑。


## 33. Agent 运行框架修订（2026-09-26）

用户确认：框架不再以「心智、潜意识、表达」固定拆分模型岗位，改为同一主体接收事件、使用能力、采取行动并根据结果继续。此节修订前文普通对话固定 Mind + Expressor 的要求；内心状态与外发内容仍保持数据边界。

普通资料充分的对话一次生成即可完成；补查记忆、浏览/工具、数据库读取等通过有界行动循环带结果续推。仅明确需要专项加工时再调用表达模型。日构建与身份整理保持后台任务，不能因聊天重构而重写其内部算法。

语音、机器人动作、Live2D 等通过能力目录和执行契约接入，可以独立发生，不以文字正文为前提。持续执行区分受理、进度、停止请求与确认终态；设备确认进入运行事件，不把模型计划当作已发生经历。身体的精确同步由组合能力负责。

当前代码、专项离线验证和未实现的设备边界见 [AGENT_HARNESS](AGENT_HARNESS.md)。本次源码改造不代表云端部署或真实设备验收。
