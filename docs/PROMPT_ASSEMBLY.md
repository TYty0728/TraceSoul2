...
输出是一张人能读完的组织卡：`beat`（当下 / 旧事 / 出门）、标签、心情、归档、新识、出门事由、当前时 `inner`、在场注意 `attention`、是否派出复盘 `review`、给外显的 `note`（不是台词）。

中枢按入口换轨：她说话走心智；`leave` 走代码外出链。定点「每日复盘」直达潜意识（现有 `identity.review`）；其余到期先叫醒心智，突发复盘由心智 `review=true` 派出。
