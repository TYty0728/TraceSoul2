...
namespace TraceSoul2.Host
{
    ...
    public sealed class SoulRuntime : IDisposable
    {
        ...
        private ILlmClient BuildNerveClient()
        {
            ...
            return new ProtectedLlmClient(new DeepSeekClientManager(config), Providers.Protection);
        }
        ...
        public Task NotifyFailuresAsync(CancellationToken token)
        {
            var adapter = liveServices.PlatformAdapters.FirstOrDefault(x => x.PlatformId == "builtin.onebot");
            return FailureNotifications.SendPendingAsync(Providers.Protection,
